...
addLayer("reset", {
    ...
    upgrades: {
        ...
        12: {
			...
			effect() {
				let mult = new Decimal(1);
				mult = mult.add(player[this.layer].points.pow(0.5).div(4)) // sqrt(x)/4
				return mult
			}, 
            effectDisplay() { return `x${format(upgradeEffect(this.layer, this.id))}` },
            cost:function(){return new Decimal("2")},
        ...
        14: {
			...
			effect() {
				let mult = new Decimal(1);
				mult = mult.add(player.points.add(1).log10().pow(3).div(2)) // (log10(x+1)^3)/2
				return mult
			}, 
            effectDisplay() { return `x${format(upgradeEffect(this.layer, this.id))}` },
...
addLayer("reb", {
    ...
    baseAmount() {return player.reset.points}, // Get the current amount of baseResource
    type: "normal", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
